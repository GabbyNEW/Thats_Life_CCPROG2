...
public class BoardGUI {
	...
	public ImageIcon careerCardFront, careerCardBack, salaryCardFront, salaryCardBack,
	...
	// South-East
	public JButton collectButton, payButton, confirmCardButton, changeCareerButton, changePathButton, payLoanButton, proceedButton;
	...
	private void setupPanelNorth() {
		...
		gridConstraints.gridy = 3;
		payRaise[0].setText("Pay Raises: " + tempPlayers[0].getCurrentPayRaise() + " / " + tempPlayers[0].getMaxPayRaise());
		...
		gridConstraints.gridy = 3;
		payRaise[1].setText("Pay Raises: " + tempPlayers[1].getCurrentPayRaise() + " / " + tempPlayers[1].getMaxPayRaise());
		...
		if (MainGame.getNumberofplayersingame() == 3) {
			...
			gridConstraints.gridy = 3;
			payRaise[2].setText("Pay Raises: " + tempPlayers[2].getCurrentPayRaise() + " / " + tempPlayers[2].getMaxPayRaise());
	...
	/**
	 * Executes if a player has not yet chosen a life path before start of the player's first round.
	 */
	...
	/**
	 * Player chose Career path (before Junction), show Career and Salary cards, allow player to throw dice.
	 * @param cards
	 */
	...
	public void setOrangeSpaceLanded(ActionCard cardDrawn) {
		...
		// SE
		if (cardDrawn.getToDoInstruction().equals("Choose A Player, then Pay $30000")) { // Allow player to choose someone to pay
			int i;
			chooseCollectFromBox.setEnabled(false);
			collectButton.setEnabled(false);
		...
		else if (cardDrawn.getToDoInstruction().equals("Choose a player, then collect $30000")) { // Allow player to choose someone to pay
			int i;
		...
		messageLabel.setText("<html>Player " + (MainGame.getTurn() + 1) + ":<br/>Action Card drawn:<br/>" + cardDrawn.getTypeOfCard() 
		+ "<br/>" + cardDrawn.getToDoInstruction() + "</html>");
		proceedButton.setText("Next Turn!");
	...
	/**
	 * Show generated number and inform if player set married is successful or rejected.
	 * Player must have already generated a number beforehand upon landing on a Get Married space.
	 * Then, allow user to throw dice again.
	...
	public void setMagentaSpaceLanded_GetMarried(int generatedNumber) {
		...
		if (generatedNumber != -1 && generatedNumber % 2 != 0) { // Player successfully set to be married (odd)
			messageLabel.setText("<html>Player " + (MainGame.getTurn() + 1) + ":<br/>Married!<br/>" 
				+ "Generated number is odd: " + generatedNumber + "<br/>Collected $5000 from other players." + "</html>");
		}
		else if (generatedNumber != -1 && generatedNumber % 2 == 0) { // Player successfully set to be married (even)
			messageLabel.setText("<html>Player " + (MainGame.getTurn() + 1) + ":<br/>Married!<br/>" 
					+ "Generated number is even: " + generatedNumber + "<br/>Collected $10000 from other players." + "</html>");
		}
		else // Player is already married (get married event rejected)
			messageLabel.setText("<html>Player " + (MainGame.getTurn() + 1) + ":<br/>Cannot marry again!<br/>" + "</html>");
	...
	/**
	 * Get chosen player from the selection boxes (XOR only one JComboBox must be active on the SE side).
